...
export class VerticalGridOperations extends GridOperations {
    ...
    public readWord(cell: Cell): Cell[] {
        ...
        let i = 1;
        let tempCell = this.grid.getCell(cell.getRow() - i, cell.getColumn());
        while (tempCell !== undefined && tempCell.getLetter() !== undefined) {
            atLeastOne = true;
            previousCells.push(tempCell);
            tempCell = this.grid.getCell(cell.getRow() - ++i, cell.getColumn());
        }
    ...
    public checkLettersAtEdges(cells: Cell[]): boolean {
        ...
        lowerEdge = this.grid.getCell(cells[0].getRow() - 1, cells[0].getColumn());
        upperEdge = this.grid.getCell(cells[cells.length - 1].getRow() + 1, cells[cells.length - 1].getColumn());
